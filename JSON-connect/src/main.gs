 ...
 *
 * @returns {Boolean}  Currently just returns false. Should return true if
 *                     the current authenticated user at the time
...
function getConfig(request) {
  ...
  config
    .newTextInput()
    .setId('token')
    .setName('Enter the Authorization token of a JSON data source')
    .setHelpText('Used in Authorization request header')
    .setPlaceholder('');
...
/**
 * Fetches data. Either by calling getCachedData or fetchJSON,
 * depending on the cache configuration parameter.
 *
 * @param   {String}   url    The URL to get the data from
 * @param   {Object}   token  The auth token to get the data from
...
function fetchData(url, token, cache) {
  ...
  try {
    ...
    sendUserError('Your request could not be executed. ' + e.message);
  }
...
/**
 * Gets UrlFetchApp response and parses JSON and checks paginated data
 *
 * @param   {string}  url    The URL to get the data from
 * @param   {Object}  token  The auth token to get the data from
 * @returns {Object}  The response object
...
function fetchJSON(url, token) {
  var data = fetchResponse(url, token);

  // data is paginated
  if (typeof data == 'object' && Array.isArray(data.results)) {
    var results = data.results;
    while (data.next) {
      data = fetchResponse(data.next, token);
...
/**
 * Gets UrlFetchApp response and parses JSON
 *
 * @param   {string}  url    The URL to get the data from
 * @param   {Object}  token  The auth token to get the data from
 * @returns {Object}  The response object
 */
 function fetchResponse(url, token) {
  try {
    var params = {};
    if (token) {
    ...
    sendUserError('"' + url + '" returned an error:' + e.message);
  }
  ...
  try {
    ...
    sendUserError('Invalid JSON format. ' + e.message);
  }
}
